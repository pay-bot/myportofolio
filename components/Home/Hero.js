...
"interactivity": {
  ...
  const [particles, setParticles] = useState(particlesOff);
  useEffect(() => {
    const ids = [
      setTimeout(() => setParticles(particlesOption), 1000),
    ];
  ...
  return (
    <>
      ...
      <Particles params={particles} className="w-full h-full absolute  " />
      <div className="lg:w-8/12 w-full flex md:flex-row flex-col lg:h-screen  mx-auto lg:px-0 md:px-8 px-4 relative lg:mt-0 mt-24 ">
        ...
        <motion.div className=" md:w-6/12 w-full items-center md:justify-start justify-center flex "
          ...
          <div className="md:text-left text-center ">
            <p className="text-5xl  md:text-6xl font-black  dark:text-white mb-3">{t("greeting.p1")}</p>
            <p className="text-4xl md:text-6xl font-black  dark:text-white">{t("greeting.p2")}<span className="text-blue-500"> Fahri</span>
            </p>

            <div className="mt-5 mb-5 text-5xl font-bold text-blue-500 ">
              <Typewriter
                options={{
